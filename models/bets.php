 ...
 * @return ?array<int,array{customer_id: string, lot_id: string, date_add: string, cost: int}
 */
...
function get_id_user_by_last_bet_on_lot(mysqli $connect, int $lot_id): ?int
{
    $sql = "SELECT user_id FROM bets 
               WHERE cost = (SELECT max(cost) from bets where lot_id = ?) and lot_id = ?";
 ...
 * @return ?array<int,array{lot_id: string, cost: int, date_add: string, date_end_lot: string,
